  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="container mx-auto max-w-6xl px-4 py-8 lg:px-8">
        ...
        {/* Progress Stepper conforming to Figma (Light grey path with Cyan checkpoints) */}
        <div className="mt-8 mb-12 relative flex items-center justify-between mx-auto max-w-4xl">
           {/* Connecting Line Track */}
           <div className="absolute left-[3%] right-[3%] top-1/2 h-[2px] -translate-y-1/2 bg-muted-foreground/15 -z-10" />
           {STEPS.map((s, i) => {
              const isCurrent = i === step;
              const isDone = i < step;
              return (
                <div key={s.label} className="flex items-center gap-2 lg:gap-3 bg-white px-2">
                   <button
                     type="button"
                     disabled={!isDone && !isCurrent}
                     onClick={() => (isDone || isCurrent) && i < 2 && setStep(i)}
                     className={`flex h-8 w-8 lg:h-9 lg:w-9 shrink-0 items-center justify-center rounded-full border-2 text-sm font-bold transition-colors ${
                       isDone
                         ? "border-primary bg-white text-primary" // Tick icon state
                         : isCurrent
                           ? "border-primary bg-primary text-primary-foreground drop-shadow-sm" // Active cyan state
                           : "border-muted-foreground/20 bg-background text-muted-foreground/50" // Future state
                     }`}
                   >
                     {isDone ? <Check className="h-5 w-5 stroke-[2.5]" /> : i + 1}
                   </button>
                   <span
                     className={`hidden text-[13px] font-bold tracking-wide sm:block ${
                       isCurrent || isDone ? "text-foreground" : "text-muted-foreground/60"
                     }`}
                   >
                     {s.label}
                   </span>
